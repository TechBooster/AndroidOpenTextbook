== XMLとJSON
WebAPIを扱う前にデータ形式について知っておく必要があります。
Webサーバーとアプリとでデータをやりとりする際のデータフォーマットとして広く使われているXMLとJSONについて説明します。

=== XML
XML(Extensible Markup Language)は1998年に標準化されたデータ形式で、アプリの設定ファイルやデータ形式として広くひろまったデータ形式です。
Androidでもstrings.xmlなどのリソースファイルやAndroidManifest.xmlで設定ファイルとして使われています。

//list[example-for-xml][XMLのサンプル]{
...
@<list>{example-for-xml} では学校に生徒が二人いて、その二人のデータが入っています。

XMLでは、"<"と">"で囲まれたタグで要素を囲います。

//list[xml-tag][XMLタグと要素]{
<count>2</count>
//}

これは生徒の数が2つあることを表しています。
閉じるタグではタグ名の前に"/"をつけます。

またタグの中に要素の付加的な情報を付けることもできます。これを属性といいます。
//list[xml-attribute][XML属性]{
<student age="18" gender="male">Taro Yamada</student>
//}
@<list>{xml-attribute} ではageとgenderがstudentの属性です。

=== JSON
JSON(JavaScript Object Notation) は2006年に標準化されたデータ形式で、当初はJavaScriptで扱うデータ形式として使われていましたが、XMLと比べ見やすいということから、いまではJavaScriptに限らず幅広い分野で使われています。

//list[example-for-json][JSONのサンプル]{
{
   "students":[
      {
         "age":"18",
...
JSONを構成するのはKeyValueのJsonObject(@<list>{json-object})と配列のJsonArray(@<list>{json-array})があります。

//list[json-object][JsonObject]{
"name":"Taro Yamada"
//}


//list[json-array][JsonArray]{
"students":[
  {
     "age":"18",
     "gender":"male",
...
WebAPIを使う時には、XMLやJSONなどのデータ形式をやりとりします。

== WebAPIにアクセスする

それではWebAPIにアクセスしてみましょう。
まずそもそもWebAPIとはなにかという話ですが、各種オンラインサービスはAPI(Application Programming Interface)を公開していることがあり、それを使うことでそのサービスが提供している情報を扱うことができるようになります。
それをWebAPIと呼びます。

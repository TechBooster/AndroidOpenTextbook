...
Androidを搭載している端末の種類はものすごい勢いで増加しています。大きさや電池の持続時間などの他に、例えば
...
作成したAndroidアプリは、様々な画面サイズやピクセル密度の端末上で実行されます。
...
Viewの幅を指定するandroid:layout_widthや、高さを指定するandroid:layout_heightは、単位付き数値（例えば100pxなど）の他に、match_parent（親Viewのサイズに合わせる）や、wrap_content（内部のコンテンツの大きさに合わせる。例えば、TextViewであれば文字列の長さや高さ）が使用できます。このmatch_parentやwrap_contentはピクセル密度を考慮にいれた上でサイズを決定するため、使用できる箇所では積極的に使用していきましょう。
...
前段でスクリーンサイズ毎の最小dp値を紹介しましたが、あくまで最小値なので、実際に販売されている機種の画面サイズ（dp単位）はこれより大きいことがほとんどです。スクリーンサイズnormalでは、長辺は最低470dp以上となっていますが、例えばGoogleのリファレンス端末であるNexus 5の長辺は690dpです。

この違いにより、「手元の開発用端末では想定していたレイアウトになるが、長辺が短い端末だと下がはみ出てしまい、一部のボタンが押せない」といった問題が発生します（図xx）

この問題を解決するため、レイアウト自体を470dp以内に収めるようやりなおすという手も考えられますが、様々な事情によりレイアウトを変更できないこともあるでしょう。その場合、レイアウト全体をScrollViewにいれてみましょう。長辺の短い機種では画面全体がスクロールするようになるため、はみ出てしまった部分が操作できないといったことは発生しなくなります。
...
この問題を解決するため、エミュレーターの起動時に、指定したインチ数でディスプレイに表示するための設定を行います。Scale display to real sizeにチェックをつけ、インチ数とディスプレイのdpiを指定してエミュレーターを起動します。この設定により、例えばディスプレイ上で4インチ（約10cm）のエミュレーターを起動することができます。

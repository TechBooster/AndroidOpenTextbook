...
本章では、作成したAndroidアプリをより多くの機種に、ユーザー体験を損なわない形で提供するための手法を紹介します。

== スクリーンサイズ/密度の違いに対応する

作成したAndroidアプリは、様々な画面サイズの端末上で実行されます。本節では、画面サイズの違いに対応するための手法を紹介します。
...
ピクセル密度に依存しない仮想的なピクセル単位（density-independent pixel）です。dp単位からピクセルへの変換は、@<m>{px = dp * (dpi / 160)}で行います。

== スクリーンサイズの最小値

各スクリーンサイズには最小値が定義されています。@<table>{screenMinsize}にグループ毎の最小値を示します。normalであれば、470dp x 320dpより大きいことが保証されています。
...
「Android搭載端末は、スクリーンサイズがばらばらで開発が困難」とよく言われますが、市販されている携帯電話の大部分はスクリーンサイズnormalに分類されます。つまり、他機種対応で最初に考慮すべき点はピクセル密度（density）の違いです。本節では、densityによって見え方／操作感の変わらないレイアウトの手法を紹介します。

==== match_parentやwrap_contentを使用する

Viewのandroid:layout_widthやandroid:layout_heightに、match_parent（親Viewのサイズに合わせる）や、wrap_content（内部のコンテンツの大きさに合わせる。例えば、TextViewであれば文字列の長さや高さ）を使用することで、ピクセル密度を気にすることなく、最適なレイアウトを行えます。
...
ここで、「拡大すると画像がぼやけてしまうのなら、逆にxxhdpi向けの画像だけ用意すれば、後は縮小してくれるのでは？」と思う方もいると思います。この場合、次のような理由により、ピクセル密度の低い端末でユーザー体験が低下してしまう恐れがあります。
...
AndroidはOSのバージョンアップに伴い、新しいAPIがどんどん追加されています。各APIは追加された時期に応じたAPI Levelが設定されています。例えば、API Level 8のAPIはAndroid 2.2で新たに追加されたAPIで、Android 2.1以前の端末では使用することができません。

作成するAndroidアプリで使用できるAPIは、原則、サポートすると決めた最低のAPI Levelまでしか使用できません。しかし、「Android 2.3をサポートしたいが、Android 4.0以上でもよりよいユーザー体験を提供したい」という理由で、API Level 14以上のAPIを使用したい、といった状況も発生します。代表的な例はNotificationです。
...
//list[api_level][API Levelを調べて分岐する例]{

//}

== 利用可能な端末を制限する

サポートする端末を増やす手法をここまで紹介しましたが、本節では、動作保証を確実にするために、インストール可能な端末を制限する方法を紹介します。
...
本章では、作成したAndroidアプリを、ユーザー体験を低下させずより多くの端末に提供するための手法を紹介しました。すべての端末をサポートするのは不可能なので、サポートする範囲を決めて、よりよいユーザー体験を提供するよう心がけましょう。

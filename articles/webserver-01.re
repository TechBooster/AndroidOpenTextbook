...
場合に寄ってはPOSTリクエストなどでデータの変更要求があることも一般的です。
...
かつては各プログラミング言語でもtelnetコマンドのような生のやりとりを実装する必要がありました。
サーバですからレスポンスヘッダをとContentを記述するわけですが、
このやりとりはプログラミングのミスをあまりにも簡単に発生させます。

curlのようにある程度HTTPのやりとりを肩代わりしてくれるライブラリのようなもの、
それがWebアプリケーションフレームワークとなります。

良いフレームワークは単純にHTTPの肩代わりをしてくれるだけではありません。
既存のWebアプリでしばしば発生するセキュリティ上の脆弱性等の問題を
予め回避したり、回避を用意にしたりする仕組みが備わっていることもあります。

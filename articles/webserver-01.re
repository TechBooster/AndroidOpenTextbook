...
IETFが公開する、@<href>{http://tools.ietf.org/}から参照出来るRFCのHTML（たとえば@<href>{http://tools.ietf.org/html/rfc7230}）からは、obsoleteとなったRFCについてはHTML上にその情報が記載されます。
...
インターネット上でそのパケットを中継するすべてのコンピュータが中身を覗き見ることが出来ます。
...
たとえばテンプレートの出来る範囲を超えて動的にHTMLを生成したい場合、

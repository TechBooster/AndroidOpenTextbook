...
というわけで、Webの世界ではURI（Universal Resource Identifier）が大変重要です。@<fn>{uri_url}

//footnote[uri_url][URL（Universal Resource Locator）という表現のほうが今でも見ることが多いのですが、現在WebサーバーやHTTPの仕様上で使われているのはURIの方です。URI、URL、そしてあまり見ることがないURNの詳細な説明はRFC 3305に記述があります。URNの説明は比較的受けることが少ないので脚注芸としてここで説明します。URNというのは例えば本のISBN番号を表現するために"urn:isbn:4844335340"と書くことができて、これは某社の某書籍を曖昧さなしに表現できます。ただしこれはWeb上のどこに情報があるかを全く意味しません。Web上の場所を指す場合にはURIを使います。urn:isbn:4844335340 をAmazon上で見るには@<href>{http://www.amazon.co.jp/dp/4844335340}をどうぞ、といった説明ができます。URNは登録制なので勝手に使えないものの、一度登録されたものについてはそれがなんであるかが曖昧さなしに決まるという特長は技術的にしばしば役に立ちます。例えばRFC1458という文字列からはRequest For Commentsの1458番という文書のことだと予想が一応立ちますが、実はラジオ福島（識別周波数が1458KHzです）のことかもしれません。urn:ietf:rfc:1458であれば絶対にラジオ福島の方を差しません。ただし、古い用法としての「URL」としての役割を果たすわけではなく、場所がわからないのでHTMLのhrefとして記述してもWebブラウザは多分どこにも移動してくれません]
...
WebサーバとHTTPを用いてやりとりするばあい、
ほとんどのケースで重要なのはURLです。

ただし、例えばAndroidでライブラリを利用する際、
URLに相当する箇所がしばしばURIになっていることもあるため、
関係は理解しておいたほうがよいでしょう。
 ...
 * 「安全なウェブサイトの作り方」
  * IPA（独立行政法人 情報処理推進機構）が公開している資料で、セキュリティを考慮したウェブサイトを作成する上で参考になる資料です。

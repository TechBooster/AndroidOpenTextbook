...
これはバイナリ型のデータフォーマットで、当然JSON本来よりは一般に効率が良いと言われています。
コンピュータ間の転送に使われるバイナリ型のデータフォーマットにはその他にもProtocol Buffersといったものもあります。
...
この方法自体は大変一般的で、本章で説明したREST的なインターフェースが発達する以前からよく行われていました。
...
//footnote[allowance][明確に許可があれば、いいのかも。先日『百姓貴族』というコミックで以下のようなエピソードを読みました。農家の人は作物を盗んだりしないのだと。そのかわり「美しいエダマメですね」「食べごろですね」「美味しそうですね」といかにも欲しそうな顔をしながら相手にコメントを述べ「持って行くかい？」という譲歩を引き出すのでした。コミックのネタを文章で説明しても面白くもなんともありませんが、許諾を取れば、今回の問題の非技術的な側面の一部は実際解決されます。大手のテーマパーク運営会社には通じなそうですけど（『百姓貴族』p39）]
...
本章全体にわたり、Webの2つ目の形態に役立つ部分を中心に説明していますが、
...
ここで述べたいのはむしろ「同じプロトコルから多様な世界が構築されている」という点です。
この世界での最下層であるHTTPやURIという概念が、だいぶ異なる用途に対しても驚くほどうまく働いている点を、
本章ではむしろ覚えておいていただきたいところです。

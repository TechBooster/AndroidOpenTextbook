...
//footnote[can_contact_using_09][一部の大手サーバでは現在でもHTTP/0.9風のレスポンスを返してくれることがあります。本稿執筆時に間違って"GET /"と送ってしまったら、同サーバからヘッダなし（0.9なので）でHTMLが返ってきました。後述するHTTP/1.1の新仕様の一つにあたるRFC 7230 Appendix（A.2）には"The expectation to support HTTP/0.9 requests has been removed."とあります。HTTPの旧仕様RFC 2616では期待してたのでしょうが、今は無視して良いようです。]
...
個別の値もさることながら、Androidアプリ開発ではHTTPレスポンスの大まかな意味がすぐにわかるものなので、知らなければこの区別だけは覚えておく方が、後々楽です。
...
//footnote[joke_rfc_is_ok_to_use][もし万が一、本当にステータスコード418 I'm a teapotを返すWebサービスがあったらどうすればよいでしょうか。実はHTTP/1.1の仕様（RFC 7230）では、クライアントに最低でも3桁の先頭の番号だけは理解するよう求めており、仮に残りの二桁で意味がわからなければx00番として解釈して良いと書かれています。ステータスコード400 Bad Requestで処理しろ、とジョークの通じないクライアントはただ判断するだけですから、少なくとも大問題にはならなそうです。]
...
その場合、レスポンス内のContentは、何らかのフォーマットで圧縮した方が良いでしょう。
...
ヘッダにContent-Lengthを埋め込むのが難しい時に
"Transfer-Encoding: chunked"を使います。@<fn>{about_chunk}
この指定をした場合、Contentには
「次に配送するデータのサイズ」
...
本章を理解できる読者ならば、新HTTP/1.1の内容は、HTTPに限らず良い勉強になるだろうと筆者は思います。
@<fn>{new_rfc_is_good}
...
過去の天気は確定したのだから、確定した天気情報を（PUTかPOSTで）更新してあげても良いかもしれません。
...
ただ、そういう事件があったことは、アプリ開発者は気に留めておくと良いでしょう。
@<fn>{good_or_bad}
...
そして、AさんがCさんにはその秘密の写真を見せても良いと、そのWebサービスに伝えていたと仮定します。
@<fn>{trip_photos}
...
なお、TLSのやりとりに関する情報も出力されるため、雰囲気はだいぶ異なります。
...
すでにURIの説明であった通り、WebリソースはURIによって一意に識別されます。
Webサーバから見ると、そのリソースを階層構造・ツリー上にマッピングする必要があります。
もしクライアントからデータを変更出来るようにする場合、
変更を行うようなpathのURIを準備するか、
RESTの原則に従うのであればGET/POST/PUT/DELETEを許すようにURIを設計し、Webサーバ上のアプリケーションを実装します。
...
脆弱性や既知のクライアント側の問題を考慮した上で、ユーザにとって満足のいく応答を返すのは一筋縄では行きません。
@<fn>{was_easy}
...
いくら仕様が目の前にあるとはいえ、自力で全てを実装するのは、通常現実的ではありません。
前もってWebにおいて発生する典型的な問題を解決する仕組みを備えた「フレームワーク」が欲しくなります。

Webアプリケーションフレームワークは、そういった問題の多くを解決する方法を提供してくれます。
例えば後述するCSRF（Cross Site Request Forgery）脆弱性にはそういった問題の典型的な解決策があります。
そして多くのフレームワークは実際に、その解決策を用意に実装する仕組みを提供してくれるわけです。

全ての脆弱性をWebフレームワークなどで防げるかと言いますと、答えは「いいえ」です。
しかし、防ぎやすい仕組みについては、他の優れたエンジニアの頭脳に頼ったほうが、
より素早く安全にWebサーバ側のアプリケーションを開発できるでしょう。
...
//footnote[need_to_modify_model][SQLが似ているとは言うものの、個別のSQL実装毎に許す文字列長などは異なり、そういった制約の違いがDjangoの実装に漏れてきてしまうことがあるのは事実です。SQLiteで動作するからと行ってMySQLに変更してそのまま運用へ投入、とはまずならないと予想されます。背後のDBのモデルを中途半端に抽象化していてびみょー、という評判が、Djangoに限らず一般的なORマッパ全体にされることもあります。]

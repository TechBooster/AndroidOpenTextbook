...
特にAndroidアプリのようなケースでは、HTMLで使われた画像をそのまま使うわけではありません。
比較的狭いスマホ画面上で適切なレイアウトをするプログラムに時間を割く必要があります。
なので、人がWebブラウザで見るためのレイアウトまで考慮されたHTMLよりも、
機械が読み取りやすいデータ形式で送ってくれた方が、それを取りに行った側のアプリでの作業は減ります。
...
//footnote[jq][curlでデバッグをしているときにはこのまま終わることはまずありません。読めないし。JSONやXMLをフォーマットしてコマンドライン上で読みやすくしたり、加工してレスポンスのこれまたJSONやXMLを自動生成したりすることがあります。本章担当者の場合、JSONだとjson_reformatやjqを使うことが多いです。]
...
まず、しばしば「RESTful」と呼ばれることのある側のRESTについて説明を試みます。@<fn>{restful}
...
ここではHTMLとそれに似ていてコンピュータが解釈しやすいXMLを同一視してしまいましょう。この際、JSONでも良いです。
...
URIというのはHTML（XML）というリソース（データ）の場所を識別するための手段です。
リソースの場所をURIに記述して、それをどうしたいかの「動詞」をHTTPリクエストとすれば、結果はHTTPのレスポンスです。

天気予報というリソースがあったとします。
天気予報サービス上のリソースというのは天気データそのものです。
地方に応じた天気があるとして、これをURIで一意に認識できるように作ることが出来ます。
たとえば@<href>{http://example.com/weather/earth/japan/tokyo/taito-ku/2014-07-11}なんてどうでしょうか。
...
すでに仕様でまとまっているので混乱は最小限で済みます。

もしあなたが管理者なら、新しい日付に対応するURIにPUTしてあげれば、翌日の天気予報データをそこに置けます。
古い天気は、DELETEで消しても良いです。
ユーザはそれ以後404を受け取りますが、「どこにあの情報あったっけ」と思うことは基本的にありません。
URIが変わらないからです。
この思想を大事にしてpath部分を設計すると、URIはいつも特定の場所の特定の日の天気を保持できます。

URIで場所を指定するということはつまり、インターネット上で巨大な木構造が一つあるというだけの話です。
天気予報サービスと書籍目録サービスがあったとして、お互いに混乱を与えることはありません。
ユーザも、ある場所にアクセスすれば同じリソースの情報があると、自信を持てます。

特にGETメソッドは、関数型言語のレベルで純粋な関数として認識できます。
...
つーわけで、WebにおいてHTTP、URI、HTML（XML、JSON）を真面目に定式化すると、
そのまま既存のプログラムと整合性が高い解釈が出来るのでした。
これをREST（Reperesentational State Transfer）と呼びます。
@<fn>{roa}
@<fn>{restructured_text}

//footnote[roa][RESTfulなWebサービスのようにリソースを中心に据えてソフトウェアの構造を考えるアプローチを「リソース指向アーキテクチャ（Resource Oriented Architecture）」と呼ぶ向きがあります。実際にはRESTful Webに関わる以外でこの表現を見ること自体まれです。]
...
//footnote[curl_is_really_useful][本当はAndroidアプリで取ってきた例を上げた方がAndroidの教科書らしいんですけど、省略します。いろいろ試す場合にそのたびにActivityを作ってアプリを起動して、とやるより、単にcurl一行でデータ取ってきて、状況に応じてPOSTリクエスト等を送るほうが、Webサービスの様子を調べる方が効率は良いような気がします。]
...
//footnote[we_will_do_that][本章後半で実際に同じようにRESTfulでないWebアプリケーションを作ります]

そもそも、元々のRoy博士の論文をベースにした「REST」を元にしても、実際に意味されるものの詳細は人によって異なることがあるようです。
今回「試みた」説明も、厳密なRESTful愛好家からすると誤りを含むものである気がします。
@<fn>{wikipedia_says}

//footnote[wikipedia_says][なるべく厳密性を逸脱せず説明をしていたつもりなのですが、純粋なRESTはハイパーテキストのやり取りをベースとする関係でデータフォーマットはXMLである必要がありそうな説明があります。ええ、マジでかよ、と思いましたが、考えてみたらそちらの方が筋は通ってます。HTTPの本来の意味論の結果がRESTだとすれば、「ハイパーテキスト」の重要な「ハイパーリンク」を取り落とすのは正確性に欠けます。]
...
特に、大量のリソースをツリー構造に並べることで、システム設計がシンプルになるのなら、RESTfulなアーキテクチャを真面目に捉える価値があると筆者は考えます。
@<fn>{cookie_and_rest}

//footnote[cookie_and_rest][RESTfulと呼ばれた場合ステートレスが基本ですので、Cookieを使ったセッションは非推奨のようです（@<href>{http://en.wikipedia.org/wiki/Resource-oriented_architecture}）。適当にURIを変えればRESTfulになるわけではないので、設計時によーく考える必要がありそうです。]
...
//footnote[xml_was_more_popular][『RESTful Webサービス』p5「プログラマブルWebはHTTPとXMLに基づいている。一部でHTML、JSON（JavaScript Object Notation）、プレーンテキスト、またはバイナリデータを扱うが、ほとんどの部分では、XMLを使用する。」とあり、読んでいてむしろ驚きました。邦訳は2007年です。]

//footnote[innaccurate][実はこの説明自体は不正確のようです。JSONのRFC 7159自体にはこのようなことは書かれていません。ただし、JSONを複数の異なる環境で共有する際にはdouble(64bitsの倍精度浮動小数点)を必要とし、さらに実装によっては数値データを整数と区別せず一律doubleのみで実装してしまった結果、あろうことか整数の範囲が52bits（doubleの仮数部）で表現される範囲に限定された、という実務上の非常に微妙な制約があるのは事実です。そしてそれが多数派であるため、実際にここに書かれている批判が妥当性を持つのでした。私の説明も不正確かもしれませんので、何度も書きますが、疑問があったらとりあえずRFCから読みましょう]

この話を更に発展させることが出来ます。
HTTP使わずに独自プロトコルの方が効率的な通信を出来るはずです。
TCP/IPでは再送制御の分だけプロトコルが重くなってますので、UDPや独自の方法で適宜処理したほうが効率が良いのではないか、等々。

通信プロトコルについてのこのたぐいの批判は、理屈上はみな正しいとも言えますが、通常、あまり現実的とは言えません。
...
そして、納得づくでそれを使ったり、実際問題があるのに入れ替えられて使われたりします。
...
ただし、それが唯一最高の方法と言えるわけではありません。

さて、上記Erlangのカウボーイは同じメッセージでケースバイケースだけどmsgpack（MessagePack）を使うことが多い、とも記述しています。
このmsgpackは、おそらく本コラムと同様の疑問の一つを解決してくれるバイナリ型のデータフォーマットです。
この他にはGoogleが開発したProtocol Buffersもあります。@<fn>{asn1}

//footnote[asn1][またもっと昔からASN.1のような汎用的な定義方法がありました。筆者はインターネット上でもデジタル証明書のDERフォーマットで、ASN.1記法を目にしたことがあります。]

TCP/IP、HTTP、JSON（XML）の組み合わせが普通使われますし、十分いろいろなことができますが、
自分のサービスを他者から区別する要素となりえるのであれば、プロトコルの再設計は選択肢の一つ「かも」しれません。
...
人が読むWebページ（HTML）からパターンを抽出して、クライアント側が本当に欲しい情報を取り出すことを「ウェブスクレイピング」と呼びます。
GoogleのようなWeb検索サービスを作る場合、なにはともあれWeb上からHTMLを収集してその中から意味のある情報を抽出する必要があります。
アプリを開発する上でもしばしば便利と感じることがあるでしょうし、達成したいアプリケーションの性質上必須であることもよくあります。
...
しかしアプリの場合、このアクセスを5秒おきに行うといったことは簡単に出来ます。

要は機械が人間の代わりに「F5アタック」をアプリが行い続けるような実装をすることができます。
サーバ管理者からすると、行い方次第では大変迷惑な行為になります。
@<fn>{rest_is_exactly_same}

//footnote[rest_is_exactly_same][実際には、RESTなインターフェースでJSON等を取得するケースでも、似たような注意は成り立ちます。アクセスし過ぎはいわゆるDOSアタックに近いお話ですので]

2つ目の問題は、REST的なインターフェースを提供していないWebサービスというのは、そもそもコンピュータが自動アクセスすることを期待していない、ということです。
...
こういうケースでは、あくまで「来場者や来場予定者がWebブラウザでちらちら見る」といった範囲に限定して、
利便性を向上させる意図でHTMLの公開をしている可能性があります。
そしてその場合、運営者は意図を超えての情報利用は嫌がるでしょう。
...
一つ目も問題なのですが、2つ目の問題で、世間の常識と技術の常識が乖離していたことから逮捕騒動に至った事例もあります。
...
これはHTTPを経由して、HTMLではなくAdobe社独自のデータ構造をクライアントに送信し、それを再生するソフトウェアをWebブラウザに組み込んでおくことで、
データを独立したアプリとして再生させる方法です。
この方法は人気があるのですが、そのようなプラグインを導入する方法がない場合にはそもそも打つ手がありません。
また、プラグインが取り扱うデータがしばしばプロプライエタリであったり（特定の会社が仕様を隠し持っていたり）することがあるため、その面でWebで嫌がられることがあります。
本章ではこの方式の説明は省略します。

2つ目はAndroidアプリのように「独立したアプリを作る」方法です。
その他にもネイティブアプリ、という表現で語られるものはだいたいここに入るでしょう。
優れた点は、最適な見栄えや、そのOSが提供する機能を存分に使えることです。
REST等のAPIを使えばWeb上のデータと連携することも出来ます。
課題があるとすれば、このタイプのソフトウェアは他のOSでは動かないことです。
また、OS特有の機能を使うということは、OSのバージョンが上がらない端末で、
ソフトウェアが使えるOS側APIが著しく制約を受ける中でソフトを作る必要が発生したりすることも意味します。

3つ目はWebサーバからHTMLとともにJavaScriptといったプログラミング言語で書かれたアプリケーションそのものを動的にダウンロードし、
Webブラウザ内でプラグインの助けなしにアプリケーションを動かすモデルです。
俗にHTML5と呼ばれるアプリの場合には、このカテゴリに属します。
良い点は、そのたびに最新のアプリケーションを受け取れることや、
サーバからするとクライアントの状況に合わせて様々なカスタマイズを動的に提供する余地が大きい点が挙げられます。
OS間差異も、ブラウザのバージョンがわかっていれば、吸収出来ることもあります。

実際にはこの3つを明確に分離して語るのは無理があります。
例えば3つ目の例で、特定のブラウザが先進的な機能を先に実装してWebサーバと連携した場合、それは1つ目と大差のない問題を抱えていることがあります。
@<fn>{dhtml}
2つ目のAndroidアプリでも、WebViewだけのアプリは3つ目のアプリに似てくることもあります。
1つ目のプラグインの話でも、RESTのようなサービスはそのアプリ内で普通に使われます。

//footnote[dhtml][DHTMLが頭をよぎりました。仕様が公開されているとは言えWebRTCなどもブラウザ間の足並みがまだ揃っていないという意味では現状では少し状況が似ているような気がしないでもありません。Ajaxの原動力となっているXMLHttpRequestは誰かが承認を与えて全員が実装して発達したものではないですから、このあたりをどう考察しても、正直後付けでしょうかね]

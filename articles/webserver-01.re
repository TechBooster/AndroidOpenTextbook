...
みんなで議論してみんなでルール（プロトコル）を決め、みんなで一斉に守ります。@<screw>
...
//footnote[experimental_rfc7238][このRFCだけexperimentalです。HTTPレスポンスコード301も本文で述べた通り"Permenanent Redirect"なのですが、その際にサーバがPOSTリクエストをGETリクエストに書き換えてしまうといった、当初の想定と異なる実装が多いので、再度厳密に定義したものです。ただ、知らないサーバの方が多いので、運用に注意しつつ試す、という意味合いが込められているようです。気になる方はRFC本文を読みましょう]
...
しかし、すでに述べた通り、HTTPはすでに大変複雑な技術と化しています。
更に、サーバへアクセスしてくるWebブラウザ側の挙動に問題があったりするため自力で全てを実装するのは難しくなっています。

特に高度なことを達成しようとすればするほど、独自実装にミスが潜みます。

また、特にセキュリティについて言えば、いくつかの典型的な問題向けには取るべき定番の対策があります。
...
全ての脆弱性をWebフレームワークなどで防げるかと言いますと、答えは「いいえ」ですが、
防ぎやすい仕組みについてはフレームワーク等を頼ったほうがより素早く安全に開発できます。

現在ではWebサーバをHTTPのレイヤから全て自前で実装することは大変稀です。
そのかわり、各プログラミング言語にはすでにWebサーバ上で動作するアプリケーションを書くフレームワークがあります。

今回はその中で、プログラミング言語Pythonとその上で動作するWebフレームワークDjangoという組み合わせにより、

...
しかし一般には、相手は別の会社のサーバだったり政府のそれだったりするので、ルールに基づいてアクセスしないと、無視されたり、あるいは悪者扱いされます。


=== コンピュータどうしのやりとりでは「プロトコル」はとても大事
...
ここで、電話は通じていることは、隠れた重要な前提です。

お店によっては、電話番号を公開してなかったり、支払いを忘れて電話が止められてたりします。
在庫確認はFAXでおねがいします、というスーパーもあるかもしれません。
直接お店に行って、もし梅干しあったら買う、という選択肢だってもちろんあります。

ここでわかるのは「問い合わせ」で使う「通信路」は複数ありえる、ということです。
そして「通信路」毎に実は個別にルール（プロトコル）があります。
例えば、FAXで問い合わせを行うのであれば、「もしもし」と言わずにFAX用紙を送る必要があります。

さて電話を使うことにしました。
スーパーなるえつに電話がつながった瞬間、相手が突然
「今朝、丸ノ内線で、すずらんの花が咲きました」と語りだしたら、客はマジで困ります。
歌を歌われたらもっと困ります。
客の我慢にも限度があります。

「電話がつながる」こと自体にルール（プロトコル）があります。
しかしそこから「その通信経路上で期待した情報を取得できる」ことの間にはまだ隔たりがあるわけです。
先ほどの例は、電話がつながることは前提ながら、
...
日本語にしても適切な応対の幅があるというのもわかります（梅干しがなければ「ない」と答えます。）

それぞれのプロトコルが成立する上では、それぞれにRFCのような仕様がある必要があります。
@<fn>{spec}

//footnote[spec][プログラムの仕様書なんて要らない、なんていう話がよくありますね。本章全体を通じて言う仕様というのはプログラムの仕様ではなく、相互にやりとりする上で必要なプロトコルの仕様のことで、目標が少し違うので注意してください。オフィスで席を並べる同僚と一緒にサーバとクライアントを作るときに、何もコミュニケーションなしに通信できることはあまりないので、そういう時に「俺はこういうデータを投げるプログラムを書くからな」という取り決めはきっとするでしょう。「言った」「言わない」問題に発展しないよう、社内Wikiにルールを書いたらそれは立派なプロトコル仕様書です。]

...
前章までの知識さえあれば、既に3DCGの複雑な描画さえ行うことが出来ます。ですが、「可能」というだけであって、それが現実的な描画速度を得られるまでには至っていませんでした。前章まででは、数万〜数十万ポリゴンにも及ぶ頂点やポリゴンを現実的な速度（１回の描画に16ms〜33ms）で行うことが出来ません。
...
GPUは"頂点バッファ"と"インデックスバッファ"を与えられると、まずインデックスバッファに対してアクセスを行います。インデックスバッファはその名の通り、index=頂点の番号が記録されています。するとGPUは、インデックスバッファに記録された番号の頂点を読み込みにいきます。
...
さらなる高速化・効率化のためには、この頂点をVRAMに格納することが重要です。今まで使用していたBufferオブジェクトはもともとCPUが効率よくデータを読み書きするためのクラスです。ですが、それはあくまでCPUにとって都合が良いというだけで、GPUにとって都合がいいとは限りません。
...
第２引数には頂点データのバイト数を指定します。今回はfloat型配列ですので、cubeVertices変数のlength×4バイトを指定すれば良いということになります。
...
OpenGL ES 2.0は現在「現役」といえるAndroid端末はほぼすべて対応しています。それに対し、OpenGL ES 3.0は一部のGPUを搭載したAndroid 4.3以上が必要で、更にAndroid Extension Packを実装した端末も存在します。

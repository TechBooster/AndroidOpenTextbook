...
[1]ではバッファオブジェクトを保持するためのメンバ変数を追加しています。実際に確保を行っているのは[2]です。バッファオブジェクトの生成は"頂点バッファ"や"インデックスバッファ"の区別なく、glGenBuffersコマンドにより行えます。引数の構成はテクスチャの生成時と同じです。テクスチャの生成は１コマンドに付き１オブジェクトしか行っていませんでしたが、今回は２つ一括で確保しています。
...
たとえば、cubeVerticesは頂点構造体により、「位置情報」「UV情報」の2つのデータが一括でアップロードされています。この時、「位置情報はバッファの何バイト目以降に入っているか」「UV情報はバッファの何バイト目以降に入っているか」を教えなければ、正しくアクセスすることが出来ません。

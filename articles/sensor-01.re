...
onAccuracyChangedはセンサーの精度が変化した時に呼ばれます。大抵の場合、onSensorChangedを実装すれば問題ありません。
...
端末の傾きを利用することで、端末の中のオブジェクトを様々な角度から見られるように変化させたり、
さらに位置情報とカメラを組みわせて、特定の場所にオブジェクトを出現させるようなARアプリの作成ができるようになります。

#@# ここの解説はGood! なにが出来るか読者に想像を促してる
...
例えば時計の秒針の角速度は、60秒で一回転(360度)なので、6度/秒です。rad = 度 × π/180となります@（@<img>{gyro}）。
...
例えば、歩数計のセンサーを必須とする場合はAndroidManifest.xmlに次のように追加します（@<list>{featues}）。

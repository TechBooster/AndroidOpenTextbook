...
//image[bt-01][Bluetooth許可リクエストダイアログ（有効設定）]{
//}
...
//image[bt-02][Bluetoothペアリング要求ダイアログ]{
//}
...
別のBluetooth機器からのデバイス検索に対して自端末を発見可能な状態にするには、「ACTION_REQUEST_DISCOVERABLE」を指定したインテントをstartAcitivityForResultメソッドで発行します。
...
//image[bt-01][Bluetooth許可リクエストダイアログ（発見可能）]{
//}
...
本章の冒頭で説明した通り、Bluetoothのバージョン4.0にはBLE（Bluetooth Low Enagy）という仕組みが採用されています。本項ではBLEの概要を説明します。
...
GATTプロファイルには、１つのプロファイルにつき複数のサービス（使用用途や機器の種類によって異なる）を持っています。また１つのサービスにつき、データ種別ごとに複数のキャラクタリスティックという単位でデータが管理されています。実際にBLE機器とデータをやり取りする場合はこのキャラクタリスティックに対してデータをやり取りします。

...
ウィザードの最後で電子署名したAPKファイルの出力先を選択します（@<img>{keystore_7}）。
Developer Consoleでは、電子署名されたAPKファイルのみ受け付けます。Google Playストアでは、開発者が前回アップロードしたアプリの署名と、次回更新するアプリの署名が
同一であることを検証しています。会社は、署名に使ったキーストアファイルとパスワードが漏れていないことを担保しなければいけません。
ファイルを紛失しないように十分気を付けてください。またセキュリティの観点からは、アプリケーションごとに異なる鍵を使って署名するとセキュリティ強度が向上します。

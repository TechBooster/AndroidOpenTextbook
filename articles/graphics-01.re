...
CPU描画の特徴は描画のほぼすべてがソフトウェアによって記述されているということです。ソフトウェア制御であることから柔軟性が高く、多くの端末で同様に動作します。

モバイル用途とは少し離れますが、ハリウッド映画等で多用される3DCGも、最終的にはソフトウェア描画によって出力が行われます。

特に、2014年現在のGPUが不得意とする「曲線描画」はCPU描画の得意分野といえます。たとえば多くの曲線によって出来上がる「文字」の描画がそれにあたります。
...
OpenGL ESは2014年現在いくつかのバージョンがあり、Android端末では（OSやGPUの制限はありますが）すべてのバージョンを使用することができます（@<table>{OpenGLES}）。
...
本書では2014年現在に普及しているほぼすべてのAndroid端末で動作可能なOpenGL ES 2.0を用いて解説を行います。
...
GLSurfaceView自体はAndroidの最初期から存在するAPIということもあり、メソッドにはすべてGL11インターフェースが第１引数として渡されます。GL11インターフェースはOpenGL ES 1.1を使用するためのクラスです。ですが今回はOpenGL ES 2.0を使用するため、GL11インターフェースの出番はありません。
...
OpenGL ES 2.0のAPIはすべて@<em>{GLES20}クラスのstaticメソッドとして実装されています。
...
これらはすべてString型の変数、つまりただの文字列としてJava言語のプログラム内に保持されています。もちろん、Javaがコンパイルされてアプリとなっても文字列は文字列のままで、このままでは実行できません。
...
ですが、まだこの状態では実行をすることが出来ません。たとえばAndroidアプリは".java"拡張子のプログラムを記述し、コンパイルによって".class"ファイルが出来上がります。実際にAndroid端末上で実行するためにはそれらを結合して".dex"ファイルに変換しなければなりません。これらは（厳密にいえば違いますが）、"コンパイル"と"リンク"という別々の作業であり、そのどちらも実行には欠かすことが出来ません。
...
すべての変数は初期状態で「使用しない」状態となるため、必要に応じてこのコマンドを呼び出す必要があります。
...
GPUは１頂点に付き、１回のmain関数が実行され、さらに頂点数だけ繰り返されます。三角形は３つの頂点で構成されるため、頂点シェーダーの呼び出し回数は３回です。attribute変数の内容はmain関数呼び出しの度に「頂点０のデータ」「頂点１のデータ」「頂点２のデータ」...という具合に変化します。
...
次に行うのが、attribute変数と頂点データの関連付けです。頂点データは非常に大きいデータ（たとえば、最近のゲームでは1キャラクターに付き数万の頂点が使用されます）であるため、一括でGPUが処理できません。
...
OpenGL ESは四角形以上のn角形の描画も行うことが出来ます。たとえば、四角形は2つの三角形に分割できますし、五角形は3つの三角形に分割することが出来ます。サンプルでは三角形を2つ描画することで、四角形を構築しています。
...
VRAMはJavaの仮想マシンとは別に確保されるため、たとえば数百MBもの量を使用しても仮想マシンがOutOfMemoryによって強制終了される心配はありません。
...
glBindTextureの第１引数には"GL_TEXTURE_2D"を指定します。第２引数には、使用を開始するテクスチャオブジェクトを指定します。また、第２引数に"0"を指定することで「使用を終了するよ」というのを明示できます。
...
頂点単位で渡されたUV座標は、varying変数を通してフラグメントシェーダーへと渡されます。フラグメントシェーダーはそのUV座標から、実際に「テクスチャの色」を取得しなければなりません。UV座標からテクスチャの色を取得する関数が"texture2D"です。第１引数にはアクセスするテクスチャを、第２引数にはUV座標をそれぞれ指定します（@<list>{vary_uvVertex}、@<list>{vary_uvFragment}）。

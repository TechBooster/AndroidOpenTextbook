...
このサンプルコードを見て気づいたかもしれませんが、今回のサンプルではOpenGL ES 2.0コマンドの呼び出しから"GLES20."の部分が省略されています。これはJavaの機能で、次のように記述することでstaticメソッドやstatic変数へのアクセスを容易に行えます。

"GLES20."を省略することで、多くのOpenGL ES関連書籍と同様の構文で記述でき、今後の理解もより深まるでしょう。

//list[][GLES20クラス内の全staticメソッドとフィールドにアクセスする]{
import static android.opengl.GLES20.*;
//}
...
アップロードは第１引数がuniform変数のLocation、第２〜第５引数がそれぞれX/Y/Z/W要素に渡される値です。XYZWはgl_FlagColorに書き込まれるとRGBAそれぞれの色として認識されます。ここではRGBAそれぞれR(1.0f)G(1.0f)B(0.0f)A(1.0f)となり、赤い色をアップロードしています。
...
glUniformXXXXコマンドは変数の型ごとに用意されており、今回宣言している"unif_color"変数はvec4型であるため、glUniform4fコマンドを使用します。この他にvec3型の変数をアップロードするglUniform3fコマンドや、後述するmat4型の変数をアップロードするための"glUniformMatrix4fv"コマンド等があります。
...
OpenGL ES 2.0では、頂点の内容をプログラマが自由に決めることが出来ます。サンプルでの頂点データは「1つの頂点は浮動小数点型のX座標とY座標を持つ」という内容になっています。三角形ですので頂点は3つ無ければなりません。

合計すると、2要素(XY)×3頂点=6つのデータがあるということです。

それを定義しているのが[11]のposition配列です。配列のindex0とindex1が頂点0、index2とindex3が頂点1、index4とindex5が頂点2をそれぞれ構成します。この時、頂点データは「1つの配列」でなければなりません。
...
第1引数には関連付けるattribute変数を指定します。

第2引数は頂点の要素数を指定します。ここではXYの2要素ですので、"2"を指定します。

第3引数は頂点の型を指定します。今回はfloat型ですので、それを示す定数のGL_FLOATを指定します。

第4引数は頂点データの正規化有無を指定します。これは省メモリ・速度重視の設計では重要になりますが、本書では説明しません。GL_FLOATを使う限り、この引数は"false"を指定すれば問題ありません。

第5引数には頂点間のオフセット(byte単位)を指定しますが、現在は0を指定すれば問題ありません。この引数についての詳細は後述します。
...
Bufferオブジェクトの生成自体は次のようになります。

//list[][float配列からBufferオブジェクトの生成]{
...
ここまできたら後一息で、描画コマンドを発行すれば完了です。描画を行うためには、[12]のようにglDrawArraysコマンドを呼び出します。

第1引数は頂点をどのようにつなげて三角形を構築するかを指定します。これは2つ以上の三角形を効率的なメモリ量・速度で描画するのに役立ちますが、サンプルでは最も基本的な形である"GL_TRIANGLES"となります。

第2引数と第3引数は、「何番目の頂点から何個分の頂点を使用するか」を指定します。今回は0番目の頂点から3個分の頂点で描画を行わせますので、それぞれ"0"、"3"となります。この時、第3引数が3未満だと描画が正常に行えません。なぜなら、「三角形を構築するには最低でも3つの頂点が必要」だからです。
...
glVertexAttribPointerは全く変わりありません。なぜなら、glVertexAttribPointerコマンドには「いくつの頂点を渡すか」という情報は付随しないからです。そのため、頂点数が6個でも、12個でも、更に増えて数万あったとしてもこのコマンドには影響がありません。

glDrawArraysコマンドの第3引数は"6"に変更されています。これは四角形＝三角形2つで構築される形状であるため、6つの頂点を描画に使用することを示しています。
...
OpenGL ESによって生成されたテクスチャオブジェクトはシェーダーのようにint型の変数によって"ID"が管理されています。

//list[][テクスチャオブジェクトはint型変数で管理される]{
    /**
     * テクスチャオブジェクト
     */
    protected int texture;
...
テクスチャオブジェクトの生成はglGenTexturesコマンドで行います。このコマンドは第1引数に「何枚のテクスチャを確保するか」を指定し、第2引数には戻り値の格納先となる配列を指定します。最後に、第3引数で「配列のどのindexから使用するか」を指定します。特別な理由がない限り、第3引数は常に0で問題ありません。

Android SDKに組み込まれたOpenGL ESのコマンドは、多くの場合この「配列のどのindexから使用するか」を指定することになります。これはJava言語がポインタ（メモリ）を直接扱えないための救済措置とも言えますが、本書では特別なことをしないため全て"0"になります。
...
glBindTextureの第1引数には"GL_TEXTURE_2D"を指定します。第2引数には、使用を開始するテクスチャオブジェクトを指定します。また、第2引数に"0"を指定することで「使用を終了するよ」というのを明示することが出来ます。
...
頂点単位で渡されたUV座標は、varying変数を通してフラグメントシェーダーへと渡されます。フラグメントシェーダーはそのUV座標から、実際に「テクスチャの色」を取得しなければなりません。UV座標からテクスチャの色を取得する関数が"texture2D"です。第1引数にはアクセスするテクスチャを、第2引数にはUV座標をそれぞれ指定します。

...
単刀直入に言えば、OpenGL ESは主に3D描画を行うためのAPIです。それを応用することで2D描画を行うことが出来ますし、CPU描画やその他の技術を組み合わせることで文字列の描画もこなすことができます。
...
ですが、まだこの状態では実行をすることが出来ません。例えばAndroidアプリは".java"拡張子のプログラムを記述し、コンパイルによって".class"ファイルが出来上がります。実際にAndroid端末上で実行するためにはそれらを結合して".dex"ファイルに変換しなければなりません。これらは（厳密に言えば違いますが）、"コンパイル"と"リンク"という別々の作業であり、そのどちらも実行には欠かすことが出来ません。

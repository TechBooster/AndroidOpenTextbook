...
//image[001][activity_main.xmlをGraphical Layoutで開く]{
//}
...
//image[006][プレビューエリアのボタンのラベルも「送信」に変わる]{
//}
...
パーツは、パレットエリアにカテゴリごとにフォルダ分けされています。他のフォルダの中も見てみましょう（@<img>{007}～@<img>{015}）。
...
//}
テキストビューやボタン、トグルボタン（ON/OFFの切り替えをするもの）、チェックボックス、ラジオボタン、プログレスバー（円形にぐるぐる回るものや水平線状のもの）、シークバー（スライドバーともいう）、レーティングバーやスイッチなどが入っています。

//image[008][Text Fields]{
//}
いろいろな形式のテキストフィールドが入っています。

//image[009][Layouts]{
//}
レイアウトに関するパーツが入っていますが、Graphical Layoutだけではちょっと使いづらいもの、もしくは使えないものですので、XMLといっしょに学習していきましょう。

//image[010][Composite]{
//}
同じようにレイアウトに関するパーツが入っていますが、プログラムと連携しなければならないものが多いです。

//image[011][Images & Media]{
//}
画像の表示や、音声や動画を表示するものが入っています。

//image[012][Time & Date]{
//}
時計やカレンダー、また日時を指定するピッカーが入っています。

//image[013][Transitions]{
//}
動きに関するパーツが入っていますが、プログラムと連携しなければならないものが多いです。

//image[014][Advanced]{
//}
その他のパーツが入っていますが、Graphical Layout単独では動かないものばかりです。「ZoomControls」などはそのまま貼り付けることができますが、このままだと触っても何も起こりません。
...
//footnote[edittextemail][通常の英数字キーボードには、「@」は出ておらず、Shiftキーなどを押して切り替えると出てくる場合が多いのですが、Emailを入力するときには「@」が必須なため、あらかじめ表示されているほうが親切です]
...
//image[022][ラジオボタンをそれぞれ単独で選ぶことができる状態。こうならないように注意]{
//}

...
(A)パレットの「Form Widgets」の中から「Button」を選んでドラッグアンドドロップし、Buttonを配置しました。
...
パーツは、パレットエリアにカテゴリごとにフォルダ分けされています。他のフォルダの中も見てみましょう。
...
//footnote[edittextemail][通常の英数字キーボードには、「@」は出ておらず、Shiftキーなどを押して切り替えると出てくる場合が多いが、Emailを入力するときには「@」が必須なため、あらかじめ表示されているほうが親切]
